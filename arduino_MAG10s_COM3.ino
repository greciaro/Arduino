...
void loop() {
  ...
  if (statusSensor == 1)
  {
    ...
    delay(400);  // motor will continue rotating for 0.4 seconds
    motor.detach(); // telling the motor to stop
    delay(10000);  // motor will remained stopped for 10 seconds
    motor.attach(7);  //re-attach the servo to pin #7
    motor.write(180); // telling the motor to rotate continously
    delay(660);  // motor will continue rotating for 0.66 seconds
    motor.detach(); // telling the motor to stop
